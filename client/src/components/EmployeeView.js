...
function getRetiringTime(employee) {
  ...
  const dob = doj.set({ year: DateTime.now().year }).minus({ years: age });
  const retirement = dob.plus({ years: RETIREMENT_AGE });

  const timeTill = retirement
    .diff(DateTime.now(), ['years', 'months', 'days'])
    .toObject();

  return `${Math.floor(timeTill.days)} days, ${timeTill.months} months, ${
    timeTill.years
  } years`;
}
